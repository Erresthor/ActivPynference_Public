...
from pyai.neurofeedback_run import save_model_performance_dictionnary,generate_instances_figures
...
from pyai.models_neurofeedback.article_1_simulations.climb_stairs_b_flat_trueA_gaussian import nf_model,evaluate_container
...
def generate_a_parameter_list(a_priors,b_priors) :
    ...
    for ka in range(len(a_priors)):
        for kb in range(len(b_priors)):
            modelchar = [False,a_priors[ka],10,True,b_priors[kb],1,True,MemoryDecayType.NO_MEMORY_DECAY,2000]
...
if __name__=="__main__":
    ...
    prior_value_b = [1.0,1.2,1.4,1.6,1.8,2.0,2.2,2.4,2.6,2.8,3.0,3.2,3.4,3.6,3.8,4.0,"perfect"]
    prior_value_a_sigma = list(np.arange(0,5.25,0.25))
    prior_value_a_sigma.append("perfect")

    parameter_list,index_list = generate_a_parameter_list(prior_value_a_sigma,prior_value_b)
    ...
    model = nf_model(model_name,save_path,prop_poubelle = prop_poubelle,
                        learn_a = a_learn,prior_A_sigma = a_acc,prior_a_strength=a_str,
    ...
    # trial_plot_from_name(save_path,model_name,2,[1,4,10,99])
    # plt.show()