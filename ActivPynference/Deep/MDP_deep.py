...
class MDP_deep_OPTIONS :
    def __init__(self):
        self.GAMMA = False      # If true, the policy precision won't be infered in all iteration
        self.PREC_A = False     # If true, the sensory precision won't be infered in all iteration
...
class MDP_deep :
    ...
    def __init__(self,*args):
        ...
        if (len(args)<1) : #Initialize an agent
            ...
            # Sensory / Transition Precision parameters
            self.beta_A_ = None  # priors for precisions to be infered (1 for each factor / modality)
            self.beta_B_ = None  # 
            
            self.BETA_A_ = None # GT precisions
            self.BETA_B_ = None # If filled, the precision will not be infered, but directly set to 1
            ...
            # Precisions of perception and transitions
            #If self.beta_A_ / beta_B_ are filled in, the infered precision are saved here
            self.beta_A = []    # Posterior over precision, if we have to infer  it
            self.beta_B = []
            self.beta_A_n = [] # Neuronal encoding of sensory precision
            self.beta_B_n = [] # Neuronal encoding of transition precision
            ...
            # If deep dims > 1 :
            # If not filled, the precision is known. s2 will likely not change ? 
            # If filled,     the precision will be infered and will be used to infer upper states
            # TODO : be able to choose which factor precision is infered
            
            # If deep dims = 1:
            # If not filled, the precision is known and set to self.BETA_
            # If filled ,    the precision is unknwown and will be infered. The value is used as a prior.
            
            self.beta_A_ = None  # priors for precisions to be infered (1 for each factor / modality)
            self.beta_B_ = None  #  If not filled, there will be no attempt at precision inference.
            ...
            self.BETA_A_ = None # GT precisions (generative process)
            self.BETA_B_ = None # If not filled, the precision will not be infered, but directly set to 1
    ...
    def copy(self):  
        """ Manual copy of the model to prevent copy of references only, which would prove desastrous for further calculations"""
        returnmdp = MDP_deep()
        parent_mdp = self
    ...
    def initialize_model_vars(self):
        ...
        if (self.beta_A_):
            self.beta_a_n = []           #Neuronal encoding of sensory precision
        if (self.beta_B_):
            self.beta_b_n = []           # Neuronal encoding of transition matrices precision
...
#                                                                 [predictive_observation_posterior] + Expected_states[:])
                ...
                for iteration in range(Ni):
                    ...
                    if (self.options.GAMMA):
                        ...
                        eg = np.dot((qu-pu).T,Q[p]) # Affective Charge (C. Hesp 2021)
                        dFdg = qb - self.beta + eg  # rate of change of beta
                        qb = qb - dFdg/2.           # update of beta posterior
                        w[t] = 1/qb                 # update of gamma posterior))
...
#        plt.show()
    ...
    def show(self):
        print(self.o)
        basic_autoplot(self.X[0][1,:])
        basic_autoplot(self.archive[0][1,int(self.T/4),:])
#        basic_autoplot(self.archive[0][1,int(self.T/2),:])
#        basic_autoplot(self.archive[0][1,int(3*self.T/4),:])
        basic_autoplot(self.o[0])
