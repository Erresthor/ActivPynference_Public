...
if __name__=="__main__":
    save_path = os.path.join("C:",os.sep,"Users","annic","Desktop","Phd","TEMPORARY_TEST_BED","article_plots")
    overwrite = True
    Ntrials = 150
    model_name = "perfect"
    model = nf_model(model_name,save_path,prop_poubelle=0.3,
    ...
    model_name2 = "imperfect_a_3"
    model2 = nf_model(model_name2,save_path,prop_poubelle = 0.3,
    ...
    model2.initialize_n_layers(10)
    model2.run_n_trials(Ntrials)
    complete_data = True
    var = True 
    save_model_performance_dictionnary(save_path,model_name2,evaluate_container,overwrite=overwrite,include_var=var,include_complete=complete_data)
    full_dico2 = load_model_performance_dictionnary(save_path,model_name2,var,complete_data)
    
    model_name3 = "imperfect_a_1.5"
    model3 = nf_model(model_name2,save_path,prop_poubelle = 0.3,
                learn_a = True,prior_a_ratio = 1.5,prior_a_strength=1.5,
                learn_b= True,prior_b_ratio = 1.0,prior_b_strength=0.1,
                learn_d= True,
                perfect_a = False,perfect_b=False,verbose = False,SHAM="False")
    model3.initialize_n_layers(10)
    model3.run_n_trials(Ntrials)
    complete_data = True
    var = True 
    save_model_performance_dictionnary(save_path,model_name3,evaluate_container,overwrite=overwrite,include_var=var,include_complete=complete_data)
    full_dico3 = load_model_performance_dictionnary(save_path,model_name3,var,complete_data)
    ...
    fig1 = figure_from_dico(full_dico)
    fig2 = figure_from_dico(full_dico2)
    fig2 = figure_from_dico(full_dico3)
