...
from tools import gaussian_to_categorical,clever_running_mean
...
def observation_gaussian_weights(No,Ns,
    ...
    for index, value in np.ndenumerate(a0[0,...]):
        reduced_current_coordinates = [index[i] for i in target_dims]
        target_coordinates = [((Ns[k]+i) if (i<0) else i) for k,i in enumerate(target_state)]

        if (method=="linear"):
            dist = np.linalg.norm(np.array(reduced_current_coordinates)-np.array(target_coordinates))
            dist_array[index] = dist
    
    projected_to_outcome_space = (1.0 - (dist_array/np.max(dist_array)))*(No-1)

    for index, value in np.ndenumerate(a0[0,...]):
        feedback_for_this_state = gaussian_to_categorical(np.zeros(No,),projected_to_outcome_space[index],sigma)
        state_obs_dist = (slice(None),)+index       
        a0[state_obs_dist] = feedback_for_this_state
    return a0

def weights_layer(Ns,Nos,
    ...
    for No,sigma,target in zip(Nos,sigmas,list_of_targets):
        ...
        if mode=="model":
            a.append(10*observation_gaussian_weights(No,Ns,sigma,target_dim,target_state,method="linear")+1.0)
...
def nf_net(T,Th,
        Ns_proc,Ns_subj,
        Nos,sigmas_proc,sigmas_subj,list_of_targets,
    ...
    A,B,_,D,_,U = weights_layer(Ns_proc,Nos,sigmas_proc,list_of_targets,
                    N_up_actions,N_down_actions,N_neutral_actions,
                    p_decay,p_effect,
                    transition_concentration,transition_stickiness,
                    allowable_actions_based_on="dim",mode="process")
    
    a,b,c,d,e,_ = weights_layer(Ns_subj,Nos,sigmas_subj,list_of_targets,
                    N_up_actions,N_down_actions,N_neutral_actions,
                    p_decay,p_effect,
                    transition_concentration,transition_stickiness,
