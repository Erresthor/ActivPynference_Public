...
from pyai.models_neurofeedback.article_1_simulations.bagherzadeh.orientation_vs_intensity import neurofeedback_model_3,neurofeedback_model_intensityuseless
...
def plot_states_trials(s_R,s_L,T,Ntrials):
    ...
    avg_value_article = 0.7392 # aai
    avg_corresponding_theta = 0.81
    std_value_article = 0.04
    color_artic = np.array([0,0,0,0.3])
    
    
    ax_right_artic = axes[0,0].twinx()
    ax_right_artic.set_ylim([-100,100])
    arr_artic = np.zeros(Ntrials)
    std_artic = np.zeros(Ntrials)
    for t in range(Ntrials):
        arr_artic[t] = 100*avg_value_article
        std_artic[t] = 100*std_value_article
    ax_right_artic.fill_between(across_trials,arr_artic-std_artic,arr_artic+std_artic,color=color_artic)
    ax_right_artic.plot(across_trials,arr_artic,color='black')
    ...
    ax_left_artic = axes[1,0].twinx()
    ax_left_artic.set_ylim([100,-100])
    arr_artic = np.zeros(Ntrials)
    std_artic = np.zeros(Ntrials)
    for t in range(Ntrials):
        arr_artic[t] = 100*avg_value_article
        std_artic[t] = 100*std_value_article
    ax_left_artic.fill_between(across_trials,arr_artic-std_artic,arr_artic+std_artic,color=color_artic)
    ax_left_artic.plot(across_trials,arr_artic,color='black')
    
    fig.suptitle("Left vs Right attention state evolution for both groups")
    axes[0,0].set_ylabel("Spatial attention orientation",fontsize = 15,color="blue")
    axes[1,0].set_ylabel("Spatial attention orientation",fontsize = 15,color="red")
    ...
    ax_left_artic.set_ylabel("Experimental avg feedback",fontsize = 15)
    ax_right_artic.set_ylabel("Experimental avg feedback",fontsize = 15)
    ...
    axes[0,0].set_yticks([0,0.5,1], ['LEFT', 'MIDDLE', 'RIGHT'],color="blue")  # Set text labels.
    ax_right_artic.set_yticks([-100,0,100], ['0', '0', '100'])
    ax_left_artic.set_yticks([-100,0,100], ['0', '0', '100'])  # Set text labels.
    axes[1,0].set_yticks([0,0.5,1], ['LEFT', 'MIDDLE', 'RIGHT'],color="red")  # Set text labels.
...
def plot_states_timesteps(s_R,s_L,T,Ntrials):
    ...
    for inst in range(Ninst):
        ...
        axes[0,0].scatter(across_timesteps,orientation_accross_timesteps,marker='+',s=3)#,color=color_rightP)
        intensity_accross_timesteps = np.average(s_R[inst,:,1,:],axis=0)/2.0
        axes[0,1].scatter(across_timesteps,intensity_accross_timesteps,marker='+',s=3)#,color=color_rightP)
    ...
    for inst in range(Ninst):
        ...
        axes[1,0].scatter(across_timesteps,orientation_accross_timesteps,marker='+',s=3)#,color=color_rightP)
        intensity_accross_timesteps = np.average(s_L[inst,:,1,:],axis=0)/2.0
        axes[1,1].scatter(across_timesteps,intensity_accross_timesteps,marker='+',s=3)#,color=color_rightP)
...
if __name__ == "__main__":
    ...
    overwrite = False 
    index = "002"
    savepath = os.path.join("C:",os.sep,"Users","annic","Desktop","Phd","TEMPORARY_TEST_BED","wide_ori_shift_true_b_uncertain" + index)
    
    p_i = 0.25 # probability of lowering the attentional level in case of monitoring attention
    p_o = 0.75 # probability of centering the directional attention in case of monitoring intensity
    feedback_depends_on_attention = True

    modelnameR = "model_rnt"
    save_pathR = savepath
    modelR = neurofeedback_model_3(modelnameR,save_pathR,p_i,p_o,'right',
                                    perfect_a=False,prior_a_precision=1,
                                    perfect_b = False,prior_b_precision=5,
    ...
    save_pathL = savepath
    modelL = neurofeedback_model_3(modelnameL,save_pathL,p_i,p_o,'left' ,
                                    perfect_a=False,prior_a_precision=1,
                                    perfect_b = True,prior_b_precision=5,
                                    perfect_d=True,prior_d_precision=3,
                                    verbose=True)
    modelL.layer_options.T_horizon=2
