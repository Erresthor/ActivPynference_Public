function [MDP] = spm_MDP_VB_XX_mod(MDP,OPTIONS)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)                  % number of MDPs
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_XX_mod(MDP(:,i),OPTIONS);
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        [G,Q(m,:),F] = spm_forwards_full(...
            O{m}(:,t),Q(m,:),A(m,:),B(m,:),C(m,:),E{m},H(m,:),W(m,:),t,T,min(T,t + N),t);
        
        disp("Computed EFE for t = " + string(t))
        disp(G')
...
% auxillary functions
%==========================================================================
function [G,P,F] = spm_forwards_full(O,P,A,B,C,E,H,W,t,T,N,t_source)
...
if t < N
    ...
    for k = 1:size(B,2)                      % search over actions
        if u(k)                              % evaluating plausible paths
            ...
            %  evaluate expected free energy for ALL hidden states
            %--------------------------------------------------------------
            j     = find(Q{k} > -1.0);
            ...
            for i = j(:)'
                ...
                % end
                P{t + 1} = Q{k};
                EF       = spm_forwards_full(O,P,A,B,C,E,H,W,t + 1,T,N,t_source);
