...
def transition_weights_centered(Ns,
    ...
    # Let's assume that for each state factor, there are :
    # - N_up_actions to increase the related state dimension
    # - N_down_actions to decrease the related state dimension
    # - N_useless_actions that don't affect the related state dimension
    # The parameters of interest here are the decay and transition probabilities. To make them 
    # differentiable, we compose the matrices as follow : 
    #     
    
    
    # The transition weights are a mixture of a number of components :
    # - A flat prior representing general uncertainty regarding state transitions
    # - A "stickiness" factor representing actions not changing one's cognitive state
    # - A "drift" factor representing how the cognitive activity of the subject spontaneously drifts towards a resting state
    # - An "effect" factor representing how specific mental actions can actually affect the cognitive state of the subjects
    
    # Each of these mixtures have associated "weights", with the first two affecting the subject generative model, and the latter two the generative process
    ...
    for f,ns_f in enumerate(Ns):
        ...
        Nu = N_up + N_down + N_neutral  # For this specific state
        base_f = np.zeros((ns_f,ns_f,Nu))
        
        base_b.append(base_f)
    
    
    spontaneous_drift = []
    
    # Decay : when no action is selected, subjects drift towards a resting mental state !
    for b_mat,idx in enumerate(base_b):
        
        
        # What is the resting state for the subject ?
        try :
            resting_state = resting_state_per_factor[f] 
        except :
            resting_state = 0

        stick_matrix = np.zeros_like(b_mat)
        decay_matrix = np.zeros_like(b_mat)
        ...
        for action in range(Nu):
            for from_state in range(ns_f):
                if (from_state == resting_state):
                    ...
                    stick_matrix[to_state,from_state,action] = 1.0
                else : 
                    # Increase of decrease ?
                    to_state = int(from_state + math.copysign(1,resting_state-from_state))
                    decay_matrix[to_state,from_state,action] = 1.0
                    stick_matrix[from_state,from_state,action] = 1.0
...
if __name__ == "__main__":
    ...
    r = discretize_normal_pdf(1.0,3.0,10,0.0,10.0)
    print(r)
