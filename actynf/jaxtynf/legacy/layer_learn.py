...
def get_delta_b(hist_u,hist_qs):
    post_qs = hist_qs[1:,:]
    pre_qs = hist_qs[:-1,:]
...
def learn_b_factorized(hist_u_tree,hist_qs_tree,pb_tree,lr_b,
    ...
    # For now, we assume a single generalizer object across all factors
    def learn_b_factor(hist_u_factor,hist_qs_factor,b_factor,linear_state_space_f):
        if linear_state_space_f:      
            db_f = get_delta_b(hist_u_factor,hist_qs_factor) 
            
            db_f = extrapolate_deltab(db_f,
                                      generalize_state_function,generalize_action_table,
                                      cross_action_extrapolation_coeff,
                                      option_clip)
        else:
            db_f = get_delta_b(hist_u_factor,hist_qs_factor) 
        
        return b_factor + db_f*lr_b
...
def learn_after_trial(hist_obs_vect,hist_qs,hist_u_vect,
          ...
          assume_linear_state_space=False,
    ...
    if learn_what["b"]:
        ...
        CLIP_ACTION_GENERALIZATION = False
        b = learn_b_factorized(u_hist_all_f,qs_hist_all_f,pb,learn_rates["b"],
                               assume_linear_state_space,
                               generalize_state_function,
                               generalize_action_table,cross_action_extrapolation_coeff,
                               option_clip=CLIP_ACTION_GENERALIZATION)
