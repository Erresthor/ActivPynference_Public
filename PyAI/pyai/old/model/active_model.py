...
class ActiveModel():
    ...
    def add_n_trials(self,n,overwrite=True):
        ...
        for ni in range(n):
            for k in range(len(self.layer_list)):
                ...
                existbool = self.save_manager.check_exists(k,lay.trial_counter,'f',lay) # True if it already exists
                if not(existbool) or (overwrite) :
                    ...
                    print("------")
                    print("Simulating Model " + str(self.model_name) +" -  Trial " + str(lay.trial_counter) +" for instance " + str(k)+ ".")

                    for ol in ActiveModel.layer_generator(lay):
                        t = ol[1]  # To get the actual timestep 
                        updated_layer = ol[0]
                    
                    verbose = False
                    if (verbose):
                        print("Action 0 matrices")
                        print(lay.B_)
                        print(type(lay.B_))
                        print("Action selection")
                        print(lay.u)
                        print(np.round(lay.u_posterior,2))
                        print(np.round(lay.Q.T,2))
                        print("State / obs")
                        print(np.round(lay.Q.T,2))
                        print(lay.o)
                        print(lay.s)
                    self.save_manager.save_process(lay,lay.trial_counter,k,'f')
                    # Save the trial AFTER the learning step !
                    
                    tafter = time.time()
                    format_float = "{:.2f}".format(tafter-tbefore)
                    print(" Simulated trial in " + format_float + " seconds.")
                    lay.trial_counter = lay.trial_counter + 1
                else : 
                    tafter = time.time()
                    format_float = "{:.2f}".format(tafter-tbefore)
