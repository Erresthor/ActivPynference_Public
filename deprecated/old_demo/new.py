...
if __name__=="__main__":
    save_path = os.path.join("C:",os.sep,"Users","annic","Desktop","Phd","TEMPORARY_TEST_BED","article_plots","4_4")
    overwrite = True
    Ntrials = 50 

    b_val = 1

    lambdas = [0.1,1,4,9,99]
    dicos = []

    model_name = "feedback = "+str("nolearn")+"_"+str(b_val)
    model = nf_model_imp5(model_name,save_path,prop_poubelle = 0.3,
            learn_a = True,prior_a_ratio = 1.1,prior_a_strength=0.1,
            learn_b= True,prior_b_ratio = 5,prior_b_strength=10,
            learn_d= True,
            perfect_a = False,perfect_b=False,verbose = False,SHAM="False")
    model.initialize_n_layers(5)
    ...
    trial_plot_from_name(save_path,model_name,0,[0,1,2,3],old=False)
    # trial_plot_from_name(save_path,model_name,0,[0,1,2,3,145],old=True)
