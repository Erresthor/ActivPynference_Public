...
from pyai.models_neurofeedback.article_1_simulations.variable_feedback_quality import nf_model,ModelOptions
...
if __name__=="__main__":
    save_path = os.path.join("C:",os.sep,"Users","annic","Desktop","Phd","TEMPORARY_TEST_BED","feedback_state_var_feed_qual")
    overwrite = True
    Ntrials = 150
    model_name = "cortico_23_fig2"

    list_of_simulations = np.arange(0.0,5.0,0.5)
    print(list_of_simulations)
    def spread_it(array1d,window_size,cut_on_limit=True):
        return_array1d = np.zeros(array1d.shape)
        for i in range(array1d.shape[0]):
            min_limit = int(i-window_size/2)
            if (i-window_size/2 < 0):
                min_limit = min_limit - 1
            max_limit = int(i+window_size/2)+1
            # print((str(min_limit) + " < " + str(i-window_size/2)+ " < " + str(i+window_size/2)+ " < " +str(max_limit)))
            sum = 0
            total_cnt = 0
            for k in range (min_limit,max_limit + 1): # k in [min_limit, max_limit]
                
                if (k>=0) and (k<array1d.shape[0]) : # k within matrix
                    if (k == min_limit): # k is at min_limit
                        prop_minlimit_in_mean = (1-(i-window_size/2 - min_limit))
                        sum = sum + prop_minlimit_in_mean*array1d[k] 
                                    # weighted average to ensure possible float scope size
                        total_cnt = total_cnt + prop_minlimit_in_mean
                    elif (k == max_limit) : # k is at max_limit
                        prop_maxlimit_in_mean = (1-(max_limit-(i+window_size/2)))
                        # print("Superior")
                        # print(k,(max_limit-(i+window_size/2)))
                        sum = sum + prop_maxlimit_in_mean*array1d[k] 
                                    # weighted average to ensure possible float scope size
                        total_cnt = total_cnt + prop_maxlimit_in_mean
                    else :
                        sum = sum + array1d[k]
                        total_cnt+= 1
                elif (cut_on_limit):
                    # print("Cut : " + str(k))
                    # Assume there are other weights of value 0 off limits :
                    if (k == min_limit): # k is at min_limit
                        prop_minlimit_in_mean = (1-(i-window_size/2 - min_limit))
                        # print(prop_minlimit_in_mean)
                        total_cnt += prop_minlimit_in_mean
                    elif (k == max_limit): 
                        prop_maxlimit_in_mean = (1-(max_limit-(i+window_size/2)))
                        # print(prop_maxlimit_in_mean)
                        total_cnt += prop_maxlimit_in_mean
                    else : 
                        total_cnt += 1

                     
                    
            return_array1d[i] = sum/total_cnt
        # print(array1d)
        # print(return_array1d)
        return return_array1d
            # array1d[i] = 



    for feedback_val in list_of_simulations: 
        options_1 = ModelOptions()
        ...
        for i in range(a.shape[0]):
            a[:,i]= spread_it(a[:,i],feedback_val)
        print(normalize(a))
        # option_1.
    ...
    # def inverto(listo):
    #     return (1.0 - np.array(listo))
    
    # error_states = inverto(error_states)
    # error_behaviour = inverto(error_behaviour)
    # error_observations = inverto(error_observations)

    # error_states_ok = inverto(error_states_ok)
    # error_behaviour_ok = inverto(error_behaviour_ok)
    # error_observations_ok = inverto(error_observations_ok)

    # error_states_imp = inverto(error_states_imp)
    # error_behaviour_imp = inverto(error_behaviour_imp)
    # error_observations_imp = inverto(error_observations_imp)

    # error_states_sham = inverto(error_states_sham)
    # error_behaviour_sham = inverto(error_behaviour_sham)
    # error_observations_sham = inverto(error_observations_sham)
    ...
    # # print(np.array(error_obs_mean_imp))
    # # print(inverto(error_obs_mean_imp))
    
    # # state_error_mean = inverto(sliding_window_mean(error_states,smooth_window))
    # # behaviour_error_mean = inverto(sliding_window_mean(error_behaviour,smooth_window))
    # # error_obs_mean = inverto(sliding_window_mean(error_observations,smooth_window))

    # # state_error_mean_ok = inverto(sliding_window_mean(error_states_ok,smooth_window))
    # # behaviour_error_mean_ok = inverto(sliding_window_mean(error_behaviour_ok,smooth_window))
    # # error_obs_mean_ok = inverto(sliding_window_mean(error_observations_ok,smooth_window))

    # # state_error_mean_imp = inverto(sliding_window_mean(error_states_imp,smooth_window))
    # # behaviour_error_mean_imp = inverto(sliding_window_mean(error_behaviour_imp,smooth_window))
    # # error_obs_mean_imp = inverto(sliding_window_mean(error_observations_imp,smooth_window))

    # # state_error_mean_sham = inverto(sliding_window_mean(error_states_sham,smooth_window))
    # # behaviour_error_mean_sham = inverto(sliding_window_mean(error_behaviour_sham,smooth_window))
    # # error_obs_mean_sham = inverto(sliding_window_mean(error_observations_sham,smooth_window))
