...
from pyai.models_neurofeedback.article_1_simulations.climb_stairs_flat_priors import nf_model_imp7,evaluate_container
...
if __name__=="__main__":
    save_path = os.path.join("C:",os.sep,"Users","annic","Desktop","Phd","TEMPORARY_TEST_BED","one_more")
    overwrite = True
    Ntrials = 50
    ...
    str_b = [1]
    str_a = [1]
    feedback_quality = [1.5,5,100]

    dicolist = []
    for fq in feedback_quality :
        for bacc in acc_b :
            for aacc in acc_a :
                for bstr in str_b :
                    for astr in str_a :
                        model_name = "feedback_"+str(fq)+"_"+str(bacc) +"_"+str(aacc) 
                        model_name += "_"+str(bstr)+"_"+str(astr)

                        model = nf_model_imp7(model_name,save_path,prop_poubelle = 0.3,
                                learn_a = True,prior_a_ratio = aacc,prior_a_strength=astr,
                                learn_b = True,prior_b_ratio = bacc,prior_b_strength=bstr,
                                learn_d = True,
                                perfect_a = False,perfect_b=False,verbose = False,SHAM="False",noiselevel=fq)
                        ...
                        dicolist.append([full_dico,model_name])
    
    for little_dic in dicolist :
        dic = little_dic[0]
        fig = figure_from_dico(dic)
        fig.suptitle(little_dic[1])
