...
class layer_input : 
    def __init__(self,layerpointer):
        self.layer = layerpointer
        self.connections = [] # pointers from outputs
    ...
    def no_input(self):
        return (self.fixed_input_empty() and self.dist_input_empty())
    
    def __str__(self):
        string_val = ""#"####################################################\n"
        string_val += ">>Input of layer " + self.layer.name+ " : \n"
        if (isField(self.o)):
            string_val +=" - o : \n"
...
class layer_output : 
    def __init__(self,layerpointer):
        self.layer = layerpointer
        self.connections = [] # pointers to inputs
        ...
        self.qu = None  # Sequence of infered action distributions [0,...,T-2]
        self.qs = None  # Sequence of infered states distributions [0,...,T-1]
        self.qo = None  # Sequence of infered observation distributions [0,...,T-1]
    ...
    def fixed_output_empty(self):
        observation_input = isField(self.o)
        state_input = isField(self.s)
        action_input = isField(self.u)
        return not(observation_input or state_input or action_input)

    def dist_output_empty(self):
        observation_input = isField(self.qo)
        state_input = isField(self.qs)
        action_input = isField(self.qu)
        return not(observation_input or state_input or action_input)

    def no_output(self):
        return (self.fixed_output_empty() and self.dist_output_empty())
    
    def __str__(self):
        string_val = ""#"####################################################\n"
        string_val += ">> Output of layer " + self.layer.name + " : \n"
        if (isField(self.o)):
            string_val +=" - o : \n"
            string_val += str(np.round(self.o,2))
            string_val += "\n----------- \n"
        ...
        if (isField(self.qo)):
            string_val +=" - q_o : \n"
            string_val += str(np.round(self.qo,2))
            string_val += "\n----------- \n"
        if (isField(self.qu)):
            string_val +=" - q_u : \n"
            string_val += str(np.round(self.qu,2))
            string_val += "\n----------- \n"
        if (isField(self.qs)):
            string_val +=" - q_s : \n"
            string_val += str(np.round(self.qs,2))
            string_val += "\n----------- \n"
        if (self.no_output()):
            string_val += "EMPTY\n"
        return string_val
...
class layer_variables :
    def __init__(self,layer):
        ...
        for modality in range(layer.Nmod):
            ...
            a_ambiguity.append(np.sum(a_norm[modality]*nat_log(a_norm[modality]),0))
...
class mdp_layer :
    ...
    def __init__(self,name,
        ...
        self.name = name
        self.seed = in_seed
        self.layerMode = layerMode.MODEL # "model" or "process"
    ...
    def copy(self,newName=None):
        new_layer =  mdp_layer(self.a,self.b,self.c,self.d,self.e,
                         self.U,self.T,self.seed)
        if (isField(newName)):
            new_layer.name = newName
        else :
            new_layer.name = self.name+"_copy"
    ...
    def use_inputs_to_populate_STM(self):
        ...
        if (t<T-1):
            # Action inputs can only be used
    ...
    def generate_observations(self):
        ...
        if (self.STM.is_value_exists("o_d",t)):
            ...
            if (self.STM.is_value_exists("o",t)):
                ...
                o = sample_distribution(o_d) # This is a tuple
            ...
            if (self.STM.is_value_exists("x_d",t)):
                ...
                if (self.STM.is_value_exists("x",t)):
                    ...
                    x = sample_distribution(x_d) # This is a tuple
            ...
            if (generate_outcome == "from_distribution"):
                for modality in range (self.Nmod):
                    ...
                    o[modality] = sample_distribution(o_d_mod)[0]
    ...
    def belief_propagation(self):
        ...
        G,Q  = spm_forwards(list_O,P,self.U,self.var,forward_t,
                    self.T,min(self.T-1,t+self.T_horizon),tree)
    ...
    def pick_action(self):
        ...
        action_selected_tuple = sample_distribution(Ru)
    ...
    def self_contained_run(self):
        self.primeModelFunctions()
        for run in self.model_generator():
