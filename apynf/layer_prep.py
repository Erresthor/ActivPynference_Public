...
def initialize_fields(layer_input):
    ...
    if isField(layer_input.o):  # There is an "o" matrix with fixed values ?
        if (type(layer_input.o) == np.ndarray) :
            o[layer_input.o >=0] = layer_input.o[layer_input.o>=0]
        elif (layer_input.o >=0) :
            o[0,0] = layer_input.o
    ...
    # true states 
    s = np.full((Nf,T),-1)
    if isField(layer_input.s):
        if (type(layer_input.s) == np.ndarray) :
            s[layer_input.s >=0] = layer_input.s[layer_input.s >=0]
        elif (layer_input.s >= 0) :
            s[0,0] = layer_input.s
    ...
    if isField(layer_input.u):
        u_temp[layer_input.u>=0] = layer_input.u[layer_input.u>=0]
